...
contract QelsToken {
    ...
    function transferFromOwner(address to, uint256 tokens) public returns (bool);
...
//    uint256 expirationPrice; // 만기일 증권 가격
    ...
    function getInform() public view returns(string, string, uint8, uint256,
        ...
        return(secName, stockCode, duesPerYear, issueAmount, issueDate, expiryDate, underPerPrice);     
        
    }
    
    
    event QelsBuilt(address founder, address token);
    event QelsTradeSet(string secName, string stockCode, uint256 startPrice, uint8 duesPerYear, uint256 issueAmout, uint32 issueDate, uint32 expiryDate, uint32 underPerPrice);
    ...
    constructor(address _founder, address _tokenAddr,string _secName, string _stockCode, uint256 _startPrice, uint8 _duesPerYear, uint8 _fundingYears,
                uint256 _issueAmout, uint32 _issueDate, uint32 _expiryDate, uint32 _underPerPrice) public {
        owner = msg.sender;
        founder = _founder;
        ...
        emit QelsTradeSet(secName, stockCode, startPrice, duesPerYear, issueAmount, issueDate, expiryDate, underPerPrice);
        emit QelsBuilt(founder, token);
    }
    
    function investQesl(uint256 _investAmount) public returns(bool){
        token.transferFromOwner(msg.sender, _investAmount);
        return true;
    }
